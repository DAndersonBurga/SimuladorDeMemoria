...
function App() {
  ...
  const [bloques, setBloques] = useState<Memoria[][]>([]);


  useEffect(() => {
    const cantidad = procesos.reduce((acc, proceso) => proceso.estado === ESTADO.EXE ? acc + proceso.size : 0, 0);
    
    setMemoriaDisponible(100 - cantidad);
  }, [procesos])

  useEffect(() => {
    setBloques(dividirEnBloques(memoria));
  }, [memoria])
  ...
  const asignarMemoria = (proceso: Proceso) => {
    setMemoria(prevMemoria => {
      const nuevaMemoria: Memoria[] = [...prevMemoria];
  ...
  const eliminarProceso = (nombreProceso: string, procesoSize: number) => {
    ...
    for (let i = 0; i < procesosEnEspera.length; i++) {
      ...
      if(procesosEnEspera[i].size <= memoriaDisponible) {
        asignarMemoria(procesosEnEspera[i]);
